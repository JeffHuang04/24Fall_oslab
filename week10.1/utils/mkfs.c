...
blk_t *iwalk(dinode_t *file, uint32_t blk_no) {
  ...
  if (blk_no < NINDIRECT) {
    ...
    //TODO();
        ...
        uint32_t *indirect_addrs = indirect->u32buf;
        if (indirect_addrs[blk_no] == 0) {
            indirect_addrs[blk_no] = balloc();
        }
        return bget(indirect_addrs[blk_no]);
  }
  panic("file * big");
}
...
void iappend(dinode_t *file, const void *buf, uint32_t size) {
  ...
  while (written < size) {
    ...
    if (to_write > BLK_SIZE - offset) {
        to_write = BLK_SIZE - offset;
    }

    memcpy(block->u8buf + offset, (const uint8_t *)buf + written, to_write);
    written += to_write;
    file->size += to_write;
    offset = 0;
  }
