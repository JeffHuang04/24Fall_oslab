...
//static uint32_t next_mmio_base = VIR_MEM;
...
void init_page() {
  ...
  for (uint32_t vaddr = VIR_MEM; vaddr < MMIO_MEM; vaddr += PGSIZE) {
    uint32_t pde_idx = ADDR2DIR(vaddr);
    uint32_t pte_idx = ADDR2TBL(vaddr);
    if (kpt[pde_idx].pte[pte_idx].val == 0) {
      kpt[pde_idx].pte[pte_idx].val = MAKE_PTE(vaddr, 1); 
    }
  }
...
PD *vm_alloc() {
  ...
  uint32_t start_vaddr = VIR_MEM;
  uint32_t end_vaddr = MMIO_MEM;

  for (uint32_t vaddr = start_vaddr; vaddr < end_vaddr; vaddr += PGSIZE) {
    uint32_t pde_idx = ADDR2DIR(vaddr);
    uint32_t pte_idx = ADDR2TBL(vaddr);
    if (new_pgdir->pde[pde_idx].val == 0) {
      PT *new_pt = (PT *)kalloc();
      new_pgdir->pde[pde_idx].val = MAKE_PDE(new_pt, 1);
      memset(new_pt, 0, sizeof(PT)); 
    }
    PT *pt = PDE2PT(new_pgdir->pde[pde_idx]);  
    pt->pte[pte_idx].val = MAKE_PTE(vaddr, 1);
  }
  return new_pgdir;
}
...
void *mmio_map_region(uint32_t pa, uint32_t size) {
    uint32_t va_start = proc_curr()->brk;
    uint32_t va_end = va_start + ((size + PGSIZE - 1) & ~(PGSIZE - 1));
    for (uint32_t va = va_start, offset = 0; va < va_end; va += PGSIZE, offset += PGSIZE) {
        uint32_t pa_curr = pa + offset;
        PTE *pte = vm_walkpte(vm_curr(), va, 7);
        pte->val = MAKE_PTE(pa_curr, 7);
    }
    proc_curr()->brk = va_end;
