...
int e1000_transmit(const char *buf, unsigned int len){
    ...
    memcpy(&tx_data_buf[tail], buf, len);
    desc->length = len;
...
int e1000_receive(char *buf, unsigned int len) {
    ...
    //TODO();
	uint32_t tail = (e1000[E1000_RDT] + 1) % RXRING_LEN;
    struct e1000_rx_desc *desc = &rx_desc_buf[tail];
	if (!(desc->status & E1000_RXD_STAT_DD)) {
        return -1;
    }
    unsigned int pkt_len = desc->length;
    memcpy(buf, &rx_data_buf[tail], pkt_len);
