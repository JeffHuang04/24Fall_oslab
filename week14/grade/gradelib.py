...
__all__ += ["config_fresh", "run_tests_bc", "MultiProcessesTester"]
...
class MultiProcessesTester:
    ...
    def gen(self, nodes, path):
        for node in nodes:
            with open(f'{path}/{node}.out', 'w') as f:
                f.write(self.outputs[node]["out"])
        exit()

    def check(self, nodes, path=None):
        def read_exps(path):
            exps = dict()
            for node in nodes:
