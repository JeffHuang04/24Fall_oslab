...
void init_cte() {
  ...
  idt[129] = GATE32(STS_IG, KSEL(SEG_KCODE), irq129, DPL_USER);
...
void irq_handle(Context *ctx) {
  ...
  case 128 : do_syscall(ctx);
  break;
  // TODO: WEEK2 handle serial and timer
  case T_IRQ0 + IRQ_COM1: serial_handle();
  break;
  case T_IRQ0 + IRQ_TIMER: timer_handle();
  break;
  // TODO: WEEK3-virtual-memory: page fault
  case EX_PF:vm_pgfault(get_cr2(), ctx->errcode);
  break;
  // TODO: WEEK4-process-api: schedule
  case 0x81: schedule(ctx);
  break;
  default: {
